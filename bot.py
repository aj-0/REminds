...
# ---------------------------------------------------------------------------
# Time parsing / formatting  ("HH:MM" 24h stored in db)
# ---------------------------------------------------------------------------

def parse_time_input(text: str):
    """'07:00 AM' / '7:00am' / '19:00' -> '19:00' (24h) or None on failure."""
...
async def edit_reminder_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rest = args[1:]
    time_parsed = parse_time_input(" ".join(rest[:2])) if rest else None
    if time_parsed:
        task_text = " ".join(rest[2:]) or None
    else:
        time_parsed = None
        task_text = " ".join(rest)
    ...
    if ok:
        await update.message.reply_text(f"✏️ Updated reminder #{reminder_id}.")
    else:
        await update.message.reply_text(f"No reminder #{reminder_id} found for you.")
...
# ---------------------------------------------------------------------------
# Callback (button) handlers — tap to mark done / snooze
# ---------------------------------------------------------------------------
...
def build_application():
    ...
    application.add_handler(CommandHandler("add", add_reminder_cmd))
